...
export default function StakingModal({ isOpen, onDismiss, stakingInfo, userLiquidityUnstaked }: StakingModalProps) {
  ...
  // pair contract for this token to be staked
    const dummyPair = (stakingInfo.name !== 'TSU-USDC'  ?
  ...
  async function onStake() {
    ...
    if (stakingContract && parsedAmount && deadline) {
      ...
      if (approval === ApprovalState.APPROVED) {
        ...
        await stakingContract.stake(`0x${parsedAmount.raw.toString(16)}`, { gasLimit: 6500000 })
  ...
  console.log('parseAmount1',stakingInfo.stakedAmount)
        ...
        <LoadingView onDismiss={wrappedOnDismiss}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            {!stakingInfo.isTokenOnly ?
              <TYPE.body fontSize={20}>{parsedAmount?.toSignificant(4)}LP TOKEN</TYPE.body>
              :
              <TYPE.body fontSize={20}>{parsedAmount?.toSignificant(4)} TSU TOKEN</TYPE.body>
            }
          </AutoColumn>
        </LoadingView>
      )}
      {attempting && hash && (
        <SubmittedView onDismiss={wrappedOnDismiss} hash={hash}>
          <AutoColumn gap="12px" justify={'center'}>
            ...
            {!stakingInfo.isTokenOnly ?
              <TYPE.body fontSize={20}>Deposited {parsedAmount?.toSignificant(4)}LP TOKEN</TYPE.body>
              :
              <TYPE.body fontSize={20}>Deposited {parsedAmount?.toSignificant(4)}TSU TOKEN</TYPE.body>
            }
